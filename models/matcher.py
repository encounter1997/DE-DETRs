...
class HungarianMatcher(nn.Module):
    ...
    def forward_two_stage(self, outputs, targets):
        ...
        tgt_ids = torch.cat([v["labels"][v['repeat'] == 1] for v in targets])
        tgt_bbox = torch.cat([v["boxes"][v['repeat'] == 1] for v in targets])
        # first cost matrix
        C = self.get_cost(outputs, tgt_ids, tgt_bbox)
        # first matching
        sizes_nr = [len(v["boxes"][v['repeat'] == 1]) for v in targets]
        ...
        # second cost matrix
        C = self.get_cost(outputs, tgt_ids, tgt_bbox)
        sizes_r = [len(v["boxes"][v['repeat'] == 0]) for v in targets]
